...
var _mapshaper = require("mapshaper");
      ...
      return Promise.all(self.tins.forw.features.map(function (tri) {
        ...
        return Promise.all(['forw', 'bakw'].map(function (direc) {
          return new Promise(function (resolve) {
            ...
            var retXy = _mapshaper.internal.dedupIntersections(xy).reduce(function (prev, apoint, index, array) {
              if (!prev) prev = {};
...
function findIntersections(coords) {
  var arcs = new _mapshaper.internal.ArcCollection(coords);
  return _mapshaper.internal.findSegmentIntersections(arcs);
}
