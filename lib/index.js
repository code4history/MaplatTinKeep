...
import { featureCollection, lineString, point, polygon, multiLineString } from "@turf/helpers";
    ...
    calcurateStrictTinAsync() {
        ...
        return Promise.all(this.tins.forw.features.map((tri) => Promise.resolve(counterTri(tri))))
            ...
            return Promise.all(["forw", "bakw"].map(direc => new Promise(resolve => {
                const multi = multiLineString(this.tins[direc].features.reduce((prev, tri) => {
                ...
                const retXy = ks.features.reduce((prev, apoint, index, array) => {
